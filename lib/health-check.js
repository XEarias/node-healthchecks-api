...
const ScrapClient = (client) => {
    ...
    if (client instanceof pgPool || client instanceof pgClient) {
        const { host, port, database } = client.config
        config.check = 'postgres'
        config.name = 'Postgres'
        config.url = `postgres://${host}:${port}/${database}`
    } else if (client instanceof elasticClient) {
        //TODO:
    } else if (client.client instanceof elasticClient) { // wingston elastic
        const { opts: { clientOpts: { node } } } = client
        config.check = 'elasticsearch'
...
const ScrapClients = (...clients) => clients.map(( client ) => scrapClient(client))


module.exports.addChecks = addChecks;
module.exports.Check = Check;
module.exports.ScrapClient = ScrapClient;
module.exports.ScrapClients = ScrapClients;